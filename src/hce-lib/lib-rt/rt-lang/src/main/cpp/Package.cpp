...
struct Package::Impl
{
   Logger *log = Logger::getLogger("rt.Package");
   ...
   int readData(void *data, const unsigned size)
   {
      if (!tar.stream)
         return -1;
   ...
   int writeData(const void *data, const unsigned size)
   {
      if (!tar.stream)
         return -1;
   ...
   static int readCallback(mtar_t *tar, void *data, unsigned int size)
   {
      const int res = gzread(static_cast<gzFile_s *>(tar->stream), data, size);
      return res == size ? MTAR_ESUCCESS : MTAR_EREADFAIL;
   }

   static int writeCallback(mtar_t *tar, const void *data, unsigned int size)
   {
      const int res = gzwrite(static_cast<gzFile_s *>(tar->stream), data, size);
      return res == size ? MTAR_ESUCCESS : MTAR_EWRITEFAIL;
   }

   static int seekCallback(mtar_t *tar, unsigned int offset)
   {
      const int res = gzseek(static_cast<gzFile_s *>(tar->stream), static_cast<off_t>(offset), SEEK_SET);
      return res == offset ? MTAR_ESUCCESS : MTAR_ESEEKFAIL;
   }

   static int closeCallback(mtar_t *tar)
   {
      gzclose(static_cast<gzFile_s *>(tar->stream));
...
int Package::open(const Mode mode)
{
