...
DesfireLimitedCredit::DesfireLimitedCredit(Instance &bundle) : Command(bundle)
{
}

int DesfireLimitedCredit::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "limitedCredit");
   ...
   if (!file->isLimitedCreditEnabled())
      return DESFIRE_STATUS_PERMISSION_DENIED;

   // receive file data
   if (const int status = picc.decodeData(request, 4, file->commSettings, {file->writeKey(), file->readWriteKey()}); status != DESFIRE_STATUS_OK)
      return status;

   // update file
   return limitedCredit(file, picc.buffer, response);
}

int DesfireLimitedCredit::limitedCredit(FileEntry *file, rt::ByteBuffer &data, rt::ByteBuffer &response) const
{
   // read credit value
   ...
   // check limit for current transaction
   if (credit > file->allowedCreditLimit())
      return DESFIRE_STATUS_BOUNDARY_ERROR;

   // update file value
   file->limitedCredit(credit);
