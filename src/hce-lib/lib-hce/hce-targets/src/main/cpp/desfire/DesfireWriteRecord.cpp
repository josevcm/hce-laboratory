...
DesfireWriteRecord::DesfireWriteRecord(Instance &bundle) : Command(bundle)
{
}

int DesfireWriteRecord::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "writeRecord [{}]", {picc.chaining});
   ...
   if (picc.chaining == 0)
   {
      ...
      // check if record file is cleared
      if (file->backup.isValid() && file->backup.isEmpty())
         return DESFIRE_STATUS_PERMISSION_DENIED;

      // check if access is denied
      if (picc.isNeverAccessAllKeys({file->writeKey(), file->readWriteKey()}))
         return DESFIRE_STATUS_PERMISSION_DENIED;

      // check if offset and length are correct
      if (length < 1 || (offset + length > file->recordSize))
         return DESFIRE_STATUS_PARAMETER_ERROR;

      // for linear record file, check if not full
      if (file->isLinearRecordFile() && file->records() == file->recordLimit)
