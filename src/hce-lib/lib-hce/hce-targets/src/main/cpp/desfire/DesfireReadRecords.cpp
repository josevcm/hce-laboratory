...
DesfireReadRecords::DesfireReadRecords(Instance &bundle) : Command(bundle)
{
}

int DesfireReadRecords::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "readRecords [{}]", {picc.chaining});
   ...
   if (picc.chaining == 0)
   {
      ...
      if (picc.isNeverAccessAllKeys({file->readKey(), file->readWriteKey()}))
         return DESFIRE_STATUS_PERMISSION_DENIED;

      // check file limits
      if (offset >= file->records() || (offset + length) > file->records())
         return DESFIRE_STATUS_BOUNDARY_ERROR;

      // prepare data to send
      const int records = length > 0 ? length : file->records() - offset;

      // prepare data buffer
      rt::ByteBuffer data(records * file->recordSize);

      // add records in chronological order (index 0 is newest)
      for (int i = records - 1; i >= 0; --i)
         data.put(file->record(i + offset));
