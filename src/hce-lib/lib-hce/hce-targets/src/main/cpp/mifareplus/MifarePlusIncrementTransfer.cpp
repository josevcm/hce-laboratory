...
MifarePlusIncrementTransfer::MifarePlusIncrementTransfer(Instance &i) : Command(i)
{
}

// Request: [0x35][srcAddr][dstAddr][EK(value_4bytes + 12_zeros)][MAC_8] = 27 bytes
// MAC = CMAC(sessionMacKey, TI||srcAddr||dstAddr||EK(value))[0:8]
// Response: [0x90][respMAC_8]
// respMAC = CMAC(sessionMacKey, TI||0x90)[0:8]
int MifarePlusIncrementTransfer::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "incrementTransfer");
   ...
   LOG_INFO(log, "\tincrement block {} by {} -> {} written to block {}", {srcAddr, delta, result, dstAddr});
