...
MifarePlusFollowingAuth::MifarePlusFollowingAuth(Instance &i) : Command(i)
{
}

int MifarePlusFollowingAuth::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   const unsigned int ins = *request.ptr();

   if (ins == MFPLUS_CMD_FOLLOWING_AUTH_P1)
      return step1(request, response);

   if (ins == MFPLUS_CMD_FOLLOWING_AUTH_P2 && awaitingStep2)
...
int MifarePlusFollowingAuth::step1(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "followingAuth step1");
   ...
   request.skip(1);
   const unsigned int keyBlockNo = request.get() | (static_cast<unsigned int>(request.get()) << 8);
...
int MifarePlusFollowingAuth::step2(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "followingAuth step2");

   if (request.remaining() != 33)
   {
      awaitingStep2 = false;
      response.put(MFPLUS_STATUS_ERR_LENGTH);
      return -1;
   ...
   plain.put(picc.auth.ti).put(rndAr).put(rt::ByteBuffer::zero(6)).put(rt::ByteBuffer::zero(6));
   plain.flip();
   ...
   LOG_INFO(log, "\tauth OK sector={}", {picc.auth.sectorNo});
