...
MifarePlusDecrement::MifarePlusDecrement(Instance &i) : Command(i)
{
}

// Request: [0xC1][blockAddr][EK(value_4bytes + 12_zeros)][MAC_8] = 26 bytes
// MAC = CMAC(sessionMacKey, TI||blockAddr||EK(value))[0:8]
// Response: [0x90][respMAC_8]
// respMAC = CMAC(sessionMacKey, TI||0x90)[0:8]
int MifarePlusDecrement::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "decrement");
   ...
   rt::ByteBuffer expectedMac = CMAC::cmac(picc.auth.sessionMacKey, macInput, iv, CMAC::CmacAES128).slice(0, 8);
   ...
   const int32_t delta = p[0] | p[1] << 8 | p[2] << 16 | p[3] << 24;
   ...
   LOG_INFO(log, "\tdecrement block {} by {} -> {}", {blockAddr, delta, result});
