...
MifarePlusRestore::MifarePlusRestore(Instance &i) : Command(i)
{
}

// Request: [0xC2][blockAddr][MAC_8] = 10 bytes
// MAC = CMAC(sessionMacKey, TI||blockAddr)[0:8]
// Response: [0x90][respMAC_8]
// respMAC = CMAC(sessionMacKey, TI||0x90)[0:8]
int MifarePlusRestore::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "restore");
   ...
   LOG_INFO(log, "\trestore block {} to transfer register", {blockAddr});
