...
MifarePlusWrite::MifarePlusWrite(Instance &i) : Command(i)
{
}

int MifarePlusWrite::process(rt::ByteBuffer &request, rt::ByteBuffer &response)
{
   LOG_INFO(log, "write");

   // [INS][blockAddr][data_16][MAC_8] = 26 bytes
   ...
   const rt::ByteBuffer data = request.getBuffer(16);
   ...
   // verify request MAC: CMAC(sessionMacKey, TI||blockAddr||data)[0..7]
   ...
   picc.blocks[blockAddr] = data.copy();
   picc.dirty = true;

   LOG_INFO(log, "\tblock {} written", {blockAddr});
