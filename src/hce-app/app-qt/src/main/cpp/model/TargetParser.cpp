...
static QVariant missingProp(const QString &name)
{
   return QVariant::fromValue(QVariantList {name, "<not set>", true});
}
...
struct TargetParser::Impl
{
   ...
   static TargetItem *parseApplication(const QJsonObject &app)
   {
      ...
      if (isoEnabled)
      {
         if (app.contains("isoId"))
            appInfo << prop("ISO ID", QString("0x%1").arg(app["isoId"].toInt(), 4, 16, QChar('0')));
         else
            appInfo << missingProp("ISO ID");

         if (app.contains("isoName") && !app["isoName"].toString().isEmpty())
            appInfo << prop("ISO DF", app["isoName"].toString());
         else
            appInfo << missingProp("ISO DF");
      }
